...
export default function Homepage() {
  ...
  // Navigation links
  const navLinks = [
    { name: "Jobs", href: "/signin", icon: <Briefcase className="h-5 w-5" />, id: "jobs" },
    { name: "Saved Jobs", href: "/signin", icon: <Heart className="h-5 w-5" />, id: "saved" },
    { name: "Companies", href: "/signin", icon: <Building className="h-5 w-5" />, id: "companies" },
    { name: "Profile", href: "/signin", icon: <User className="h-5 w-5" />, id: "profile" },
    { name: "Career Advice", href: "/signin", icon: <FileText className="h-5 w-5" />, id: "career" },
    { name: "Resources", href: "/signin", icon: <BookOpen className="h-5 w-5" />, id: "resources" },
  ]
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden">
      ...
      {/* Header/Navigation */}
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-black/80 backdrop-blur-md shadow-lg shadow-purple-900/10" : "bg-transparent"}`}
      >
        <div className="mx-auto max-w-7xl flex items-center justify-between px-4 py-4 md:px-6">
          <div className="flex items-center">
            <Link href="/" className="flex items-center">
              <div className="h-10 w-10 rounded-full bg-gradient-to-br from-purple-600 to-purple-900 flex items-center justify-center mr-2">
                <span className="font-bold text-white">HF</span>
              </div>
          ...
          <nav className="hidden md:flex items-center space-x-1">
            {navLinks.map((link) => (
              <button
                ...
                className={`group px-3 py-2 rounded-md text-gray-300 hover:text-white hover:bg-purple-900/30 transition-all duration-300 flex items-center ${activeSection === link.id ? "bg-purple-900/40 text-white" : ""}`}
              >
                <span
                  className={`mr-2 ${activeSection === link.id ? "text-purple-300" : "text-purple-500 group-hover:text-purple-300"}`}
                >
                  {link.icon}
                </span>
                <span>{link.name}</span>
                <span
                  className={`ml-1 h-1 w-1 rounded-full ${activeSection === link.id ? "bg-purple-300" : "bg-purple-500 group-hover:bg-purple-300"} transition-all duration-300 group-hover:scale-150`}
                ></span>
              </button>
            ))}
          </nav>
          ...
          <div className="flex items-center">
            <Link
              href="/signin"
              className="hidden md:flex items-center mr-4 px-4 py-2 text-sm font-medium text-purple-300 hover:text-white border border-purple-800/50 hover:border-purple-500 rounded-md transition-all duration-300"
            >
              Sign in
            </Link>
            <Link
              href="/signup"
              className="bg-gradient-to-r from-purple-700 to-purple-900 hover:from-purple-600 hover:to-purple-800 px-4 py-2 text-sm font-medium text-white rounded-md transition-all duration-300 shadow-md shadow-purple-900/50 hover:shadow-lg hover:shadow-purple-700/50"
            >
              GET STARTED
            </Link>
            <button
              className="ml-4 rounded-md p-2 text-gray-300 hover:bg-purple-900/30 hover:text-white transition-all duration-300 md:hidden"
        ...
        >
          <div className="px-4 py-2 space-y-1 bg-black/90 backdrop-blur-md border-t border-purple-900/30">
            {navLinks.map((link) => (
              <button
                key={link.name}
                onClick={() => scrollToSection(link.id)}
                className={`flex w-full items-center px-3 py-3 rounded-md text-gray-300 hover:text-white hover:bg-purple-900/30 transition-all duration-300 ${activeSection === link.id ? "bg-purple-900/40 text-white" : ""}`}
              >
                <span className={`mr-3 ${activeSection === link.id ? "text-purple-300" : "text-purple-500"}`}>
                  {link.icon}
                </span>
                <span>{link.name}</span>
              </button>
            ))}
            <div className="pt-2 pb-3 border-t border-purple-900/30 mt-2">
              <Link
                href="/signin"
                className="flex items-center w-full px-3 py-3 text-gray-300 hover:text-white hover:bg-purple-900/30 transition-all duration-300 rounded-md"
