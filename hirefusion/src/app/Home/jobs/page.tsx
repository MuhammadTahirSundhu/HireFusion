...
export default function JobsPage() {
  ...
  // Fetch all jobs from /api/jobs/all-jobs
  ...
  const filteredResults: JobWithId[] = showSavedOnly ? results : results;
  ...
  const handleJobSelection = (localIndex: number): void => {
    const globalIndex = results.findIndex(
      (job) => job._id.toString() === filteredResults[indexOfFirstJob + localIndex]._id.toString()
    );
    setSelectedJobIndex(globalIndex);
  };
